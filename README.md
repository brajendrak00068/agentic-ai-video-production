...
- **OpenClaw:** `openclaw mcp add levea --command "npx -y levea-mcp-server" --env LEVEA_API_URL=https://api.livecore.ai --env LEVEA_API_KEY=...`
- **Hermes:** see [`hermes-levea/`](./hermes-levea/)

**3. Use it.** Once registered, say to your AI: *"Use the levea tool to generate 5 viral clips from this video, add captions, reframe vertical."* The agent picks the tool, fills the prompt, the editor plans + executes + exports.
...
- **npm (MCP server)** — [npmjs.com/package/levea-mcp-server](https://www.npmjs.com/package/levea-mcp-server)
